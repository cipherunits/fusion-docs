...
A route module typically owns one resource path stem via `[module]` (e.g. `ProductModule` → `product`). Convention handlers (`get` / `post` / …) and optional custom HTTP routes (`@http_get`, …) live on that class.
...
- HTTP handlers (`get`, `post`, `@http_get`, …)
...
| Framework packages | Published as `fusion-framework` / `FusionFramework` (currently **1.2.3** in the framework repo) |
